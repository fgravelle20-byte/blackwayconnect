...
import {
  deletePhoneAssistant,
  updatePhoneAssistant,
} from "@/modules/phone-assistance/phone-service";

const patchSchema = z.object({
  name: z.string().min(1).max(200).optional(),
  status: z.enum(["draft", "active", "paused", "integration_required"]).optional(),
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const assistant = await updatePhoneAssistant(id, organization.id, parsed.data);
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!organization) return NextResponse.json({ error: "No organization" }, { status: 400 });
  const { id } = await ctx.params;
  await deletePhoneAssistant(id, organization.id);
